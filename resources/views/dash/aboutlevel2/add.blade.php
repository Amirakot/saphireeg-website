...
@section('page-header')
				...
				<div class="breadcrumb-header justify-content-between">
					<div class="my-auto">
						<div class="d-flex">
							<h4 class="content-title mb-0 my-auto"> aboutlevel2 <i class="fa fa-level-down" aria-hidden="true"></i>  </h4><span class="text-muted mt-1 tx-13 mr-2 mb-0">/ADD</span>
						</div>
					</div>
					...
					<div class="col-lg-12 col-xl-12 col-md-12 col-sm-12">
						<div class="card  box-shadow-0">
							...
							<div class="card-body pt-0">
								<form class="form-horizontal" enctype="multipart/form-data" method="post" action="{{route('aboutlevel2.store')}}" >
                                    @csrf
...
@endif
                                    ...
                                    <div class="form-group">
                                        <select name="about_level1_id" id="" class="form-control">
                                            <option value="{{ old('about_level1_id') }}"
                                            selected disabled> --about_level1_id--</option>
                                            @foreach ($aboutlevel1 as $about)
                                                <option value="{{ $about->id }}"> {{ $about->title }}</option>
                                            @endforeach
                                            @error('about_level1_id')
                                                <div class="alert alert-danger">{{ $message }}</div>
                                            @enderror
                                        </select>
                                    </div>
