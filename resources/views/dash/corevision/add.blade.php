...
@section('page-header')
				...
				<div class="breadcrumb-header justify-content-between">
					<div class="my-auto">
						<div class="d-flex">
							<h4 class="content-title mb-0 my-auto"> corevision <i class="fa fa-level-down" aria-hidden="true"></i>  </h4><span class="text-muted mt-1 tx-13 mr-2 mb-0">/ADD</span>
						</div>
					</div>
					...
					<div class="col-lg-12 col-xl-12 col-md-12 col-sm-12">
						<div class="card  box-shadow-0">
							...
							<div class="card-body pt-0">
								<form class="form-horizontal" enctype="multipart/form-data" method="post" action="{{route('corevision.store')}}" >
                                    @csrf
...
<div class="form-group">
   <select name="vision_level1_id" id="" class="form-control">
    <option value="" disabled> -- Vision Level 1 --</option>
    @foreach ($visionlevel1 as $vision)
        @if ($vision->title === 'Core Value')
            <option value="{{ $vision->id }}" selected>{{ $vision->title }}</option>
        @endif
    @endforeach
</select>

    @error('version_level1_id')
        <div class="alert alert-danger">{{ $message }}</div>
    @enderror
</div>
