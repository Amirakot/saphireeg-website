...
@section('page-header')
				...
				<div class="breadcrumb-header justify-content-between">
					<div class="my-auto">
						<div class="d-flex">
							<h4 class="content-title mb-0 my-auto"> futurelevel1 <i class="fa fa-level-down" aria-hidden="true"></i>  </h4><span class="text-muted mt-1 tx-13 mr-2 mb-0">/ADD</span>
						</div>
					</div>
					...
					<div class="col-lg-12 col-xl-12 col-md-12 col-sm-12">
						<div class="card  box-shadow-0">
							...
							<div class="card-body pt-0">
								<form class="form-horizontal" enctype="multipart/form-data" method="post" action="{{route('futurelevel1.store')}}" >
                                    @csrf
...
@endif
                                    ...
                                    <div class="form-group">
                                        <select name="future_id" id="" class="form-control">
                                            <option value="{{ old('service_level1_id') }}"
                                            selected disabled> --future--</option>
                                            @foreach ($futures as $future)
                                                <option value="{{ $future->id }}"> {{ $future->title }}</option>
                                            @endforeach
                                            @error('service_level1_id')
                                                <div class="alert alert-danger">{{ $message }}</div>
                                            @enderror
                                        </select>
                                    </div>
